...
**Terakhir di-update:** `2026-07-19` · **oleh device:** `Victus (Windows)` · **oleh:** 06 RootFacts (Dafina) DIBUAT & terverifikasi lokal — app web Basic (CV TF.js + GenAI flan-t5-base + PWA Workbox), K1+K2+K3 lolos live via Chrome DevTools (0 error); sisa user deploy Netlify + zip + upload
...
- **Yang paling mendesak (open action, urut prioritas):**
  0. **06 RootFacts (Dafina)** — ✅ BARU dibuat & **terverifikasi lokal** (2026-07-19), target Basic. K1+K2+K3 lolos live (Chrome DevTools, 0 error). Perlu user **deploy Netlify** (akun Dafina) → isi `STUDENT.txt` → zip `RootFacts_Dafina_Meira_Rizkia.zip` (flat, tanpa `_*test*.html`) → upload. 🍎 Panduan: `dafina/panduan/Deploy_Netlify.md`.
...
| 06 | penerapan-ai-aplikasi-web | RootFacts PWA (flan-t5-base) — **Dafina** | 🚧 | target ⭐⭐⭐ (Basic) | 🍎 | Kode done + **K1/K2/K3 verified live (0 error)**. **User deploy Netlify → STUDENT.txt → zip → upload** |
