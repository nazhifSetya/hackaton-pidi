...
- **2026-07-21 (Victus):** Proyek dibuat dari nol.
  ...
  - Commit+push monorepo (main). **SISA (user Bimo):** (1) `gh auth login` (akun Bimo sendiri) → `python panduan/push_repos.py <username>` (buat 2 repo PUBLIC + push + isi 2 .txt + regen zip) → cek Actions Workflow-CI hijau ≥1×; (2) konfirmasi dashboard name = username Dicoding (skrg "bimo"); (3) upload zip ke Dicoding.
