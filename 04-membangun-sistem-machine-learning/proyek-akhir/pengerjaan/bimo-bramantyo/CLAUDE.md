...
- **2026-07-21 (Victus):** Proyek dibuat dari nol.
  ...
  - K4: exporter Flask (5 metrik `anggur_*`, port 8010) + serving verified lokal (health OK, 5 prediksi 0/0/1/2/1). Stack Docker (Prometheus :9090 + Grafana :3000) jalan, target scrape UP, **6 screenshot** diambil via chrome-devtools: `1.bukti_serving.jpg`, 4× Prometheus (permintaan/prediksi/cpu/memori), `5.../1.dashboard_bimo.jpg` (5 panel terisi). ✅
  - **Audit adversarial (agent):** vonis SIAP SUBMIT (Basic), tak ada REJECT. Temuan PENTING = `3.prometheus_exporter.py` awalnya terlalu mirip struktur+docstring punya Dafina → **DIROMBAK** (instrumentasi per-handler + `Histogram.time()` + gauge dibaca saat scrape + docstring/komentar/penamaan baru; nama metrik `anggur_*` tetap → screenshot lama tetap valid, sudah re-verified jalan). Modelling.py MINOR (sudah cukup beda: Pipeline+KONFIG+Indonesia). Serving screenshot hanya beranda = MINOR (ditopang log txt).
  - Zip `submission/SMSML_Bimo-Bramantyo.zip` (682 KB, 39 file, flat, tanpa zip-in-zip/mlruns) dibuat.
  - **Repo di-push ke akun GitHub Bimo `bimo2107` (2026-07-21):** [Eksperimen_SML_Bimo-Bramantyo](https://github.com/bimo2107/Eksperimen_SML_Bimo-Bramantyo) (PUBLIC) & [Workflow-CI](https://github.com/bimo2107/Workflow-CI) (PUBLIC). **GitHub Actions HIJAU** (run 56s, job `latih-ulang-model`, artefak `anggur-mlruns`). 2 .txt terisi URL asli, zip di-regen (682 KB). Push pakai token PAT Bimo (scope repo+workflow) via URL-embedded — token TIDAK ke-commit; **token perlu di-revoke user** (sempat terekspos di chat).
  - **SISA (user Bimo):** (1) UPLOAD `submission/SMSML_Bimo-Bramantyo.zip` ke Dicoding; (2) konfirmasi dashboard name = username Dicoding (skrg "bimo"); (3) revoke token GitHub.
  - ⚠️ Artefak hanya di Victus (git-ignored): `submission/*.zip` + `Monitoring dan Logging/model/model.pkl` (dan salinan submission). Regen: jalankan `modelling.py` → copy artifacts/model → rebuild zip.
