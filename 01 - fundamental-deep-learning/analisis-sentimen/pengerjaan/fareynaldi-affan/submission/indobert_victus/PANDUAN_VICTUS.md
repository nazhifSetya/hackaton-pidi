# 🎮 Panduan Menjalankan IndoBERT di HP Victus (RTX 3050) — Skema 4 (MyTelkomsel)

> **Kenapa di Victus?** Fine-tune IndoBERT butuh GPU (CUDA). MacBook M1 tidak punya CUDA,
> jadi bagian ini dijalankan di Victus. Hasilnya (2 file JSON) dikirim balik ke M1 untuk
> digabung ke notebook utama.

---

## Yang kamu butuhkan
- 2 file dari folder ini: **`indobert_train_victus.py`** + **`requirements_victus.txt`**
- 1 file dataset dari M1: **`dataset_mytelkomsel_labeled.csv`** (dibuat setelah tahap pelabelan)
...
- Progres: loss per step + akurasi per epoch.
...
> **Fallback total (kalau Victus bermasalah):** upload `indobert_train_victus.py` +
> `dataset_mytelkomsel_labeled.csv` ke **Google Colab** (Runtime → GPU T4 gratis),
> install requirements, jalankan. Sama saja hasilnya.
...
(Folder `indobert_mytelkomsel_model/` = model tersimpan, opsional — besar, tidak wajib dikirim.)
