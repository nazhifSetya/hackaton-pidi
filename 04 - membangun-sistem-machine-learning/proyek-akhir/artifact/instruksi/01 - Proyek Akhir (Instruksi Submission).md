# Proyek Akhir — Instruksi Submission
...
| ***WAJIB DIPERHATIKAN!*** Mohon periksa tab “Lainnya” untuk memeriksa ketentuan pengiriman submission lebih lanjut. |
| --- |
...
![Proyek Akhir (Instruksi Submission)](images/01-img01.png)

| **Catatan:** Perhitungan nilai akhir di atas digunakan apabila setiap kriteria mendapatkan nilai 2 pts atau tidak ada kriteria yang ditolak. |
| --- |
...
- Format pengiriman submission.SMSML_Nama-siswa ├── Eksperimen_SML_Nama-siswa.txt ├── Membangun_model ├── modelling.py ├── modelling_tuning.py (skilled/advanced) ├── namadataset_preprocessing (bisa berupa file atau folder) ├── screenshoot_dashboard.jpg ├── screenshoot_artifak.jpg ├── requirements.txt ├── DagsHub.txt (berisikan tautan DagsHub jika menerapkan advanced) ├── Workflow-CI.txt ├── Monitoring dan Logging ├── 1.bukti_serving ├── 2.prometheus.yml ├── 3.prometheus_exporter.py ├── 4.bukti monitoring Prometheus (folder) └── 1.monitoring_<metriks> └── 2.monitoring_<metriks> └── dst (sesuaikan dengan poin yang diraih) ├── 5.bukti monitoring Grafana (folder) └── 1.monitoring_<metriks> └── 2.monitoring_<metriks> └── dst (sesuaikan dengan poin yang diraih) ├── 6.bukti alerting Grafana (folder) └── 1.rules_<metriks> └── 2.notifikasi_<metriks> └── 3.rules_<metriks> └── 4.notifikasi_<metriks> └── dst (sesuaikan dengan poin yang diraih) ├── 7.Inference.py ├── folder/file tambahan
  - **Catatan**
...
- Kriteria 1
  ...
  - Jika Anda menerapkan **Advance**, silakan buat workflow yang sudah dijalankan dan minimal satu kali berhasil tanpa menghasilkan error.  
    ...
    ![Proyek Akhir (Instruksi Submission)](images/01-img02.jpeg)
- Kriteria 2
  - Pastikan Anda menyimpan seluruh artefak pada MLflow Tracking UI dengan alamat *localhost* atau 127.0.0.1.  

    ![Proyek Akhir (Instruksi Submission)](images/01-img03.jpeg)
  - Jika menerapkan **skilled**, pastikan Anda membuat file modelling_tuning.py dan melakukan logging model yang menghasilkan struktur seperti berikut.  
    ...
    ![Proyek Akhir (Instruksi Submission)](images/01-img04.jpeg)
  - Jika menerapkan **advanced**, pastikan Anda menambahkan minimal dua artefak selain pada tahapan **skilled**. Selain itu, Anda harus menyimpan file artefak MLflow ke DagsHub agar dapat diakses secara online.  
    ...
    ![Proyek Akhir (Instruksi Submission)](images/01-img05.jpeg)

      
    Sebagai contoh, Anda dapat menggunakan kode berikut agar dapat menyimpan artefak pelatihan ke DagsHub.import dagshub import mlflow dagshub.init(repo_owner=’<nama_owner>', repo_name='<nama_repo>', mlflow=True) with mlflow.start_run(): # Your training code here...Jika Anda belum memasukkan kredensial apa pun, silakan *login* terlebih dahulu dengan mengikuti dokumentasi [DagsHub](https://dagshub.com/docs/integration_guide/mlflow_tracking/).
- Kriteria 3
  - Silakan Anda buat sebuah repository GitHub dengan visibilitas **Public** agar bisa diperiksa oleh tim reviewer.
  ...
  - Jika Anda menerapkan **basic**, pastikan workflow CI yang dibuat memuat tahapan berikut.  
    ...
    ![Proyek Akhir (Instruksi Submission)](images/01-img06.jpeg)
  - Jika Anda menerapkan **skilled**, pastikan workflow CI yang dibuat memuat tahapan berikut.  

    ![Proyek Akhir (Instruksi Submission)](images/01-img07.jpeg)
  ...
  - Jika Anda menerapkan **advanced**, pastikan workflow CI yang dibuat memuat tahapan berikut.  
    ...
    ![Proyek Akhir (Instruksi Submission)](images/01-img08.jpeg)
...
- Kriteria 4
  - Pastikan tangkapan layar yang Anda kirim memiliki nama dashboard yang berisikan username akun Dicoding Anda seperti berikut.  
    ...
    ![Proyek Akhir (Instruksi Submission)](images/01-img09.jpeg)
  - Jika menerapkan **basic**, silakan lakukan serving model baik itu menggunakan MLflow serve, membuat API menggunakan framework, dan lain sebagainya. Namun, pastikan Anda menyertakan bukti serving seperti berikut ini.  
    ...
    ![Proyek Akhir (Instruksi Submission)](images/01-img10.jpeg)

      
    Atau ketika menggunakan Docker Images bisa seperti berikut.

    ![Proyek Akhir (Instruksi Submission)](images/01-img11.jpeg)
    - Anda harus menyertakan bukti Prometheus sudah berjalan dengan membuat minimal tiga buah metriks monitoring seperti berikut ini.  
      ...
      ![Proyek Akhir (Instruksi Submission)](images/01-img12.jpeg)
  ...
  - Jika menerapkan Alerting, silakan sisipkan dua file seperti berikut ini.
    - **Bukti rules yang dibuat**  
      ...
      ![Proyek Akhir (Instruksi Submission)](images/01-img13.jpeg)
    - **Bukti notifikasi**  

      ![Proyek Akhir (Instruksi Submission)](images/01-img14.jpeg)

## Lainnya

### Ketentuan Pengiriman Berkas Submission

- Berkas submission yang dikirimkan merupakan folder berisi kumpulan berkas yang diminta **dalam bentuk ZIP** seperti contoh berikut.SMSML_Nama-siswa.zip ├── Eksperimen_SML_Nama-siswa.txt ├── Membangun_model ├── modelling.py ├── modelling_tuning.py (skilled/advanced) ├── namadataset_preprocessing (bisa berupa file atau folder) ├── screenshoot_dashboard.jpg ├── screenshoot_artifak.jpg ├── requirements.txt ├── DagsHub.txt (berisikan tautan DagsHub jika menerapkan advanced) ├── Workflow-CI.txt ├── Monitoring dan Logging ├── 1.bukti_serving ├── 2.prometheus.yml ├── 3.prometheus_exporter.py ├── 4.bukti monitoring Prometheus (folder) └── 1.monitoring_<metriks> └── 2.monitoring_<metriks> └── dst (sesuaikan dengan poin yang diraih) ├── 5.bukti monitoring Grafana (folder) └── 1.monitoring_<metriks> └── 2.monitoring_<metriks> └── dst (sesuaikan dengan poin yang diraih) ├── 6.bukti alerting Grafana (folder) └── 1.rules_<metriks> └── 2.notifikasi_<metriks> └── 3.rules_<metriks> └── 4.notifikasi_<metriks> └── dst (sesuaikan dengan poin yang diraih) ├── 7.Inference.py ├── folder/file tambahanPastikan Anda **tidak melakukan ZIP dalam ZIP**.
...
*Sumber: [Proyek Akhir (Instruksi Submission)](https://www.dicoding.com/academies/713/tutorials/42236/submission-guidance) — Dicoding, kelas Membangun Sistem Machine Learning.*
