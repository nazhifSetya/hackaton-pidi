...
INPUT : dataset_pln_labeled.csv  (kolom: text_clean, label)  <- versi V4 (18.794 baris)
...
Skema evaluasi (BERSIH, tanpa kebocoran):
...
X = df[TEXT_COL].astype(str).tolist(); y = df["labels"].tolist()

# train/test 80/20, lalu train -> train2/val 90/10 (val utk pilih model terbaik)
Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.2, random_state=SEED, stratify=y)
Xtr2, Xval, ytr2, yval = train_test_split(Xtr, ytr, test_size=0.1111, random_state=SEED, stratify=ytr)
print(f"train={len(Xtr2)} val={len(Xval)} test={len(Xte)} device={'cuda' if torch.cuda.is_available() else 'CPU (!)'}")
...
model = AutoModelForSequenceClassification.from_pretrained(
    ...
    use_safetensors=True,          # muat bobot format .safetensors (hindari error torch.load pickle)
...
trainer = Trainer(
    ...
    processing_class=tokenizer, data_collator=DataCollatorWithPadding(tokenizer),  # 'tokenizer=' sudah deprecated
...
with open("indobert_metrics.json", "w") as f: json.dump(metrics, f, indent=2)
with open("indobert_history.json", "w") as f:
    json.dump(train_out.metrics if hasattr(train_out, "metrics") else {}, f, indent=2)
trainer.save_model("indobert_pln_model"); tokenizer.save_pretrained("indobert_pln_model")
print(f"\nSELESAI. Kirim balik indobert_metrics.json. RINGKAS -> TEST acc={te_acc:.4f} f1={te_f1:.4f}")
