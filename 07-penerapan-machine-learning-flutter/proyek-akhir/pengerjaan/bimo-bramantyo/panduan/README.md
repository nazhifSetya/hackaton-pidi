# Panduan PindaiRasa (Bimo) — Submission Flutter 07

Aplikasi **PindaiRasa**: foto makanan → aplikasi menebak namanya pakai model
...
inferensi benar **"SATAY" 98.8%** di emulator, alur K1→K3 di-screenshot). Yang
tersisa untukmu cuma **cek sekali & upload zip**.
...
- Model `assets/models/aiy_food_v1.tflite` (21 MB) **sudah ada** di folder proyek. (File ini git-ignored — tapi ikut di dalam zip. Kalau perlu ambil ulang: [Kaggle AIY Food V1](https://www.kaggle.com/models/google/aiy/tfLite/vision-classifier-food-v1), rename huruf kecil `aiy_food_v1.tflite`, taruh di `submission/pindai_rasa/assets/models/`.)

## 1. (Opsional) Coba jalankan sendiri di HP / emulator

```powershell
cd 07-penerapan-machine-learning-flutter\proyek-akhir\pengerjaan\bimo-bramantyo\submission\pindai_rasa
...
nama makanan + persen keyakinan + lima tebakan teratas (bar chart).

> Catatan: model mengenali **2.023 jenis makanan dunia**. Untuk foto makanan
> yang jelas (mis. satay, sushi, nasi lemak), nama top-1 biasanya tepat dengan
> keyakinan tinggi.
...
Zip **isi** folder `pindai_rasa` (bukan folder `submission/` di atasnya, dan
**jangan** ikutkan `panduan/`, `scratchpad/`, `CLAUDE.md`).

Cara paling aman (path forward-slash standar via bsdtar):

```powershell
cd 07-penerapan-machine-learning-flutter\proyek-akhir\pengerjaan\bimo-bramantyo\submission
tar -a -c -f pindai_rasa_Bimo.zip pindai_rasa
```

Zip target **± 18–19 MB** (di bawah plafon 25 MB). File sudah dibuatkan di
`submission/pindai_rasa_Bimo.zip` (18.84 MB).
...
Submit `pindai_rasa_Bimo.zip` di halaman submission kelas 758. (Boleh ganti nama
zip agar cocok dengan username Dicoding-mu bila diminta.)
...
| **K3 Halaman Prediksi** | Halaman hasil: foto + nama makanan + keyakinan % + bar chart lima tebakan teratas. |
